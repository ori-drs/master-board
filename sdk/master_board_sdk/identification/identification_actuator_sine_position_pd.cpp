...
int main(int argc, char **argv)
{
	...
	if(flag_logging) // if flag for logging is true
	{
		...
		{
			loggerMotor[i].createFile("identification_actuator_sine_position_pd_motor" + std::to_string(i) + ".log");
			...
			case 1:
				...
				for (int i = 0; i < N_SLAVES_CONTROLED * 2; i++)
				{
					...
					if (robot_if.motors[i].IsEnabled())
					{
                        ...
                        // if (freq >= freq_end)
                        //     freq = freq_end;
						double ref = amplitude * sin(2 * M_PI * freq * t);   // M_PI is pi from math package
                        double v_ref = amplitude * 2*M_PI*freq*cos(2*M_PI*freq*t);
