...
int main(int argc, char **argv)
{
	...
	streamObj3 << current;   				//Add double to stream
	std::string current_str = streamObj3.str();   // Get string from output string stream
	...
	{
		// loggerIMU.createFile("identification_actuator_accelerate_IMU.log");  // create logger file
		...
		{
			loggerMotor[i].createFile("identification_actuator_accelerate_HLK_uboard1b_" + current_str + "A_" + std::to_string(i) + ".log");
			...
			case 1:
				...
				for (int i = 0; i < N_SLAVES_CONTROLED * N_MOTORS_PER_BOARD; i++)
				{
					if (i % N_MOTORS_PER_BOARD == 0)
					{
   					...
   					if(abs(robot_if.motors[i].GetVelocity()) > maxVelocity) // if motor's velocity exceed the maximum allowed velocity
						state = 2;  // go to emergency stop


					if (robot_if.motors[i].IsEnabled())
					{
						double cur = current;   // M_PI is pi from math package
