...
int main(int argc, char **argv)
{
	...
	double kp = 5.;
	double kd = 0.1;
	double iq_sat_pos = 1.0;
	double iq_sat_neg = -1.0;
	double freq = 0.05;
	...
	loggerIMU.createFile("LogIdentificationActuatorSine_IMU.log");  // create logger file
	...
	{
		loggerMotor[i].createFile("LogIdentificationActuatorSine_Motor" + std::to_string(i) + ".log");
			...
			case 1:
				...
				for (int i = 0; i < N_SLAVES_CONTROLED * N_MOTORS_PER_BOARD; i++)
				{
					...
					if (robot_if.motors[i].IsEnabled())
					{
						...
						loggerMotor[i].writeMotorLog(t,robot_if.motors[i]);  // log motor i
					}
				}
				break;
			}
