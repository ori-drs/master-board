...
//					robot_if.motors[i].SetVelocityReference(0.);
            ...
            case 3:
				...
				{                    
					//if (robot_if.motors[i].IsEnabled())
					{
						...
						{
                            ref = trajectoryParameters[i].zero_pos;
                            vref = 0;
						}
                        robot_if.motors[i].SetCurrentReference(0.);
            ...
            case 4:
                ...
                break;
			case 5:
                if (t > 2.5)  // after 10 seconds switch motor
                {
                    activeMotor++;
                    state = 2;   // go back to initialisation mode to move all joints back to zero position
                }
                if (activeMotor > 11)
                    state = 6;
				...
				for (int i = 0; i < N_SLAVES_CONTROLED * 2; i++)
				{
					...
					if (robot_if.motors[i].IsEnabled() && i==activeMotor)
					{
